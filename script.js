...
// Função para lidar com o clique no botão "QUERO AGENDAR"
function queroAgendar(arenaNome, zona, bairro, horario, valor) {
    // Codifica os detalhes da arena como parâmetros na URL
    var detalhesURL = 'detalhes/detalhes.html?' +  // Ajuste o caminho conforme necessário
                      'nome=' + encodeURIComponent(arenaNome) +
                      '&zona=' + encodeURIComponent(zona) +
                      '&bairro=' + encodeURIComponent(bairro) +
                      '&horario=' + encodeURIComponent(horario) +
                      '&valor=' + encodeURIComponent(valor);

    // Redireciona para a nova página com os detalhes da arena
